...
suffix = "simplifiedMitigation" # the suffix to add to this run of the variable 
conditionVar = ["data_type.Primary", 'oro_branch.Mitigation'] # the variable(s) that has to ==1 in order to predict the target Var
...
for k, (train, test) in enumerate(outer_cv):    
    ...
    try:
        pr = param_space[0]
        cv_results=pd.read_csv(f'{dockerFilePath}outputs-docker/model_selection/{targetVar}_{suffix}__model_selection_{k}.csv').to_dict('records') #File path 1, change name
        params_tested=pd.read_csv(f'{dockerFilePath}outputs-docker/model_selection/{targetVar}_{suffix}__model_selection_{k}.csv')[list(pr.keys())].to_dict('records') #File path 2, change name
    except:
        cv_results = []
        params_tested = []
    for pr in param_space:
        ...
        pd.DataFrame.from_dict(cv_results).to_csv(f'{dockerFilePath}outputs-docker/model_selection/{targetVar}_{suffix}__model_selection_{k}.csv',index=False) #File path 3, change name
        gc.collect()
