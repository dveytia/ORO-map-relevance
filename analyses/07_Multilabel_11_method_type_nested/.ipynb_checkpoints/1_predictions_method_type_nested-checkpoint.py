...
conditionVar = "data" # the variable that has to ==1 in order to predict the target Var
...
for k in range(3):
    inner_df = pd.read_csv(f'/home/dveytia/ORO-map-relevance/outputs/model_selection/{targetVar}_{suffix}_model_selection_{k}.csv') 
...
for k, (train, test) in enumerate(outer_cv.split(seen_index)):    
    ...
    np.save(f'/home/dveytia/ORO-map-relevance/outputs/predictions/{targetVar}_{suffix}_y_preds_5fold_data_{k}.npz',y_preds) #Change file path + name

np.save(f'/home/dveytia/ORO-map-relevance/outputs/predictions_data/{targetVar}_{suffix}_data_pred_ids',df.loc[unseen_index,"id"]) #Change file path + name
