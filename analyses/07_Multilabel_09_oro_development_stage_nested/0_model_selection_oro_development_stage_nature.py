...
conditionVarVal = 'oro_branch.Nature'
...
for k, (train, test) in enumerate(outer_cv):    
    ...
    try:
        pr = param_space[0]
        cv_results=pd.read_csv(f'/home/dveytia/ORO-map-relevance/outputs/model_selection/{targetVar}_{suffix}_model_selection_{k}.csv').to_dict('records') 
        params_tested=pd.read_csv(f'/home/dveytia/ORO-map-relevance/outputs/model_selection/{targetVar}_{suffix}_model_selection_{k}.csv')[list(pr.keys())].to_dict('records')
    except:
        cv_results = []
        params_tested = []
    for pr in param_space:
        ...
        pd.DataFrame.from_dict(cv_results).to_csv(f'/home/dveytia/ORO-map-relevance/outputs/model_selection/{targetVar}_{suffix}_model_selection_{k}.csv',index=False) #File path + name 3 
        gc.collect()
