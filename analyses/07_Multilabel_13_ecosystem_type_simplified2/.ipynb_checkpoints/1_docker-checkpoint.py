...
relevanceTxt = '/home/devi/analysis/outputs/predictions-compiled/1_document_relevance_13062023.csv'
...
with open('/home/devi/analysis/pyFunctions/multi-label_1_predictions_functions.py') as f:
    exec(f.read())
...
for k in range(3):
    inner_df = pd.read_csv(f'/home/devi/analysis/outputs-docker/model_selection/{targetVar}_{suffix}_model_selection_{k}.csv') 
...
for k, (train, test) in enumerate(outer_cv.split(seen_index)):    
    ...
    np.save(f'/home/devi/analysis/outputs-docker/predictions/{targetVar}_{suffix}_y_preds_5fold_data_{k}.npz',y_preds) #Change file path + name

np.save(f'/home/devi/analysis/outputs-docker/predictions_data/{targetVar}_{suffix}_data_pred_ids',df.loc[unseen_index,"id"]) #Change file path + name
